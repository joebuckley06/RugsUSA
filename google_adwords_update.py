...
# Campaign map for Google
campaign_map = pd.read_csv('RugsUSA_Campaign_Map.csv')
google_map = campaign_map[campaign_map['Channel'].isin(['Google','Connexity','Ebay'])][['Channel','Campaign','Category']].copy()


def update_AWS():
    df_adwords = pd.read_sql_query('''SELECT * FROM google_spend;''', cnx)
    max_db_date_plus_one = str(pd.to_datetime(max(df_adwords['Date'])) + timedelta(1))[:10]
    ...
    if max_db_date_plus_one < yesterday:
        ...
        df_google = df_adwords[df_adwords['Channel']=='Google'].copy()
        df_google['Category'] = df_google['Campaign'].apply(google_categories)
        df_google_ads = df_google[(df_google['Spend']>0)].copy()
        df_google_ads['Tuesday_Week'] = df_google_ads['Date'].apply(tuesday_week)
        df_google_ads['Impressions'] = df_google_ads['Impressions'].astype(int)
        df_google_ads['Clicks'] = df_google_ads['Clicks'].astype(int)
        df_google_ads['Conversions'] = df_google_ads['Conversions'].fillna(0).astype(int)
        df_google_ads['Revenue'] = df_google_ads['Revenue'].astype(float)
        ...
        tableName = 'google_spend'

        try:
            frame = df_google_ads.to_sql(tableName, dbConnection, if_exists='append');
